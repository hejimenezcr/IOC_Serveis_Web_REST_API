...
function renderitzarLlibres(docs) {
    ...
    grid.innerHTML = docs.map(book => {
        ...
        return `
        <div class="col">
            <div class="card h-100 shadow-sm" style="cursor:pointer;" onclick="obrirDetall('${book.key}')">
                <img src="${cover}" class="card-img-top" style="height:250px; object-fit:cover;">
                <div class="card-body">
                    <h6 class="card-title fw-bold">${book.title}</h6>
                    <p class="card-text small text-muted">
                        ${book.author_name ? book.author_name[0] : 'Autor desconegut'}
                    </p>
                </div>
            </div>
        </div>`;
    ...
    try {
        ...
        <div class="row">
            ...
            <div class="col-md-7">
                <h3>${info.title}</h3>
                <p class="text-muted">
                    ${info.description?.value || info.description || 'Sense descripció'}
                </p>

                <ul class="list-group mt-3">
                    <li class="list-group-item">
                        <strong>Data:</strong> ${info.created?.value || 'N/A'}
                    </li>

                    <li class="list-group-item">
                        <strong>Temes:</strong>
                        ${info.subjects ? info.subjects.slice(0, 5).join(', ') : 'N/A'}
                    </li>
                </ul>
            </div>
        </div>`;
    ...
    for (let i = Math.max(1, currentPage - 2); i <= Math.min(totalPages, currentPage + 2); i++) {
        html += `
            <li class="page-item ${i === currentPage ? 'active' : ''}">
                <button class="page-link" onclick="canviarPagina(${i})">${i}</button>
            </li>
        `;
    }
